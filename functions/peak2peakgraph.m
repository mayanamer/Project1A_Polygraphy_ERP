...
subject_list = {'subject1_session4', 'subject3_session3', ...
    ...
    'subject4_session5', 'subject4_session2', 'subject4_session3', ...
...
result_p = getP2PVector(guilty_probe_path, honest_probe_path, subject_list, t_low, t_high, Ts, 10);

result_t = getP2PVector(guilty_target_path, honest_target_path, subject_list, t_low, t_high, Ts, 10);

result_i = getP2PVector(guilty_irr_path, honest_irr_path, subject_list, t_low, t_high, Ts, 10);

result = [result_p];%; result_t; result_i];
...
title('Peak to Peak of the Probe signals(1200 samples)')
grid on
grid minor

%%
function p2p_vec = getP2PVector(guilty_path, honest_path, sub_list, t_low, t_high, Ts, channel)
    num_signals = numel(sub_list);
    p2p_vec = zeros(num_signals*2, 1);
    ...
    for i = 1:num_signals
        ...
        signal = avg_with_channels(signal, 1.6, 0.002, [channel]);
        signal = signal(t_low/Ts:t_high/Ts);
        p2p_vec((num_signals+i),1) = peak2peak(signal);
    end
end
